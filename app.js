...
async function boot() {
  ...
  // Route: /teacher hash opens teacher login, everything else goes straight to student sign-in
...
window.requestGPS = async function () {
  ...
  try {
    ...
    // GPS obtained — proceed to code gate
    ...
    btn.disabled = false;
    if (e.code === 1) {
      errEl.textContent = "Location was denied. Please go to your browser settings, allow location for this site, then refresh.";
    } else {
      errEl.textContent = "Could not get your location. Make sure GPS is turned on and try again.";
    }
...
window.verifyCode = async function () {
  ...
  if (entered !== session.code) {
    errEl.textContent = "Incorrect code. Check the board and try again.";
    errEl.style.display = "block";
    return;
  }
...
window.openSession = async function () {
  ...
  try {
    ...
    const proceed = confirm("Could not get your location.\nThe session will open WITHOUT location verification — students anywhere can sign in.\n\nProceed anyway?");
    if (!proceed) return;
  }
  ...
  document.getElementById("t-active-sessions").innerHTML = active.map(s => `
    <div class="session-item active-sess" style="flex-direction:column;align-items:stretch">
      ...
      <div class="code-display">
        ...
        <div class="code-hint">${s.lat ? "✓ Location verification on" : "⚠ No location — code only"}</div>
      </div>
    </div>`).join("");
