...
// Update hint text when GPS toggle changes
document.addEventListener("DOMContentLoaded", () => {
  const gpsCheck = document.getElementById("t-use-gps");
  const hint = document.getElementById("gps-hint");
  if (gpsCheck && hint) {
    gpsCheck.addEventListener("change", () => {
      if (gpsCheck.checked) {
        hint.innerHTML = "Students must be within 30m of you to sign in. <strong>Disable for indoor classes.</strong>";
        hint.style.color = "rgba(255,220,140,0.9)";
      } else {
        hint.innerHTML = "✓ <strong>Code only</strong> — any student with the code can sign in regardless of location. Good for indoor classes.";
        hint.style.color = "rgba(150,255,150,0.9)";
      }
    });
  }
});
...
window.openSession = async function () {
  ...
  if (!t)    return alert("Please set a start time.");
  if (!tEnd) return alert("Please set an end time.");

  const [h, m]   = t.split(":").map(Number);
  const [he, me] = tEnd.split(":").map(Number);
  const start = new Date(); start.setHours(h, m, 0, 0);
  const lock  = new Date(); lock.setHours(he, me, 0, 0);
  if (lock <= start) return alert("End time must be after start time.");

  // Find open button safely — use id instead of class selector
  const btn = document.getElementById("open-session-btn");
  const useGPS = document.getElementById("t-use-gps").checked;
  ...
  if (useGPS) {
    ...
    try {
      ...
      setBtn("Open session", false);
      const isPWA = window.matchMedia("(display-mode: standalone)").matches || window.navigator.standalone;
      const locMsg = isPWA
        ? "Could not get your location.\n\niPhone: Settings → Privacy & Security → Location Services → find this app → set to While Using.\nAndroid: Settings → Apps → this app → Permissions → Location → Allow.\n\nOpen session with code-only verification instead?"
        : "Could not get your location.\n\nOpen session with code-only verification instead?";
      const proceed = confirm(locMsg);
      if (!proceed) return;
    }
  }
  ...
  try {
    ...
    if (lat !== null) {
      ...
      showLocStatus("✓ Location acquired — tracking live", "green");
    } else {
      showLocStatus("⚠ No location — code verification only", "amber");
    }
    // Switch to sessions view to show code immediately
    switchTab("sessions", document.querySelector(".tab"));
  } catch(e) {
    console.error("openSession error:", e);
    if (e.code === "permission-denied") {
      alert("Permission denied by Firebase.\n\nFix: Go to Firebase Console → Firestore Database → Rules → publish:\n\nallow read, write: if true;");
    } else if (e.code === "unavailable") {
      alert("Firebase is unreachable. Check your internet connection and try again.");
    } else {
      alert("Failed to open session: " + (e.message || e.code || e));
    }
  }
