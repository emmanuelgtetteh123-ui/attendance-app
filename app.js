...
async function boot() {
  ...
  // Route: ONLY go to teacher if #teacher is explicitly in the URL
  // Students always go straight to sign-in
  ...
  document.getElementById("t-active-sessions").innerHTML = active.map(s => `
    <div class="session-item active-sess" style="flex-direction:column;align-items:stretch">
      ...
      <div class="code-display">
        ...
        <div class="code-hint">${s.lat ? "✓ Location (50m) + code" : "⚠ Code only — no GPS"}</div>
      </div>
    </div>`).join("");
